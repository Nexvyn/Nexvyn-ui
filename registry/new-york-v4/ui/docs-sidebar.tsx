...
import { BookmarkCollapsible } from "@/components/docs/bookmark-collapsible"
    ...
    <div className="bg-sidebar flex h-[100dvh] w-screen overflow-x-hidden pr-2.5 pl-2.5 lg:pl-0">
      ...
      >
        <SidebarContent className="no-scrollbar px-1">
          <Link href="/" className="flex items-center px-4 pt-4 group-data-[collapsible=icon]:px-3">
            <Icons.logo className="text-foreground size-5" />
            <span className="text-foreground ml-2 font-sans text-lg font-bold group-data-[collapsible=icon]:hidden">
              {siteConfig.name}
            </span>
        ...
        <div className="flex h-15 w-full items-center justify-between py-2 lg:h-11.5 lg:justify-end">
          ...
          */}
          <div className="flex items-center">
            ...
            <div>
              <CommandMenu tree={tree} colors={colors} className="pointer-events-auto" />
            </div>
            <div className="hidden items-center lg:flex">
              <Separator orientation="vertical" className="mr-2 ml-3 !h-[20px]" />
              <DocsDropdown />
            </div>
          </div>
        </div>
