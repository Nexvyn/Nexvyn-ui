...
export interface RatioSliderProps extends Omit<HTMLAttributes<HTMLDivElement>, 'onChange' | 'defaultValue'> {
  value?: number
  defaultValue?: number
...
export const RatioSlider = forwardRef<HTMLDivElement, RatioSliderProps>(({
  ...
  useEffect(() => {
    ...
    const checkLabelFit = () => {
      ...
      setIsCompact(leftBarWidth < leftLabelWidth + edgePadding || rightBarWidth < rightLabelWidth + edgePadding)
    }
    checkLabelFit()
    ...
    >
      <div className='relative h-8 mb-4'>
        <div
          ...
          style={{ color: leftLabelColor, top: isCompact ? labelsRowHeight / 2 : labelsRowHeight + gap + barHeight / 2, transform: 'translateY(-50%)' }}
        >
        ...
        <div
          ...
          style={{ color: rightLabelColor, top: isCompact ? labelsRowHeight / 2 : labelsRowHeight + gap + barHeight / 2, transform: 'translateY(-50%)' }}
        >
      ...
      <div ref={sliderRef} className={cn('relative w-full flex items-center gap-2 touch-none', reduceMotion ? '' : 'transition-[height] duration-200 ease-out')} style={{ height: barHeight }}>
        <div
          ref={leftBarRef}
          className={cn(
...
export function RatioSliderPreview() {
  return (
    <div className='w-full h-full min-h-[200px] rounded-lg overflow-hidden flex items-center justify-center p-4' style={{ backgroundColor: 'var(--color-surface)' }}>
