...
export function StarsCount() {
  ...
  const safeAnimate = useCallback(
    async (
      ...
      if (!mountedRef.current || !scope.current) return
      try {
  ...
  useEffect(() => {
    ...
    const playBounceSequence = async () => {
      await wait(1500)
      while (isPlaying && mountedRef.current && scope.current) {
        await safeAnimate(
          '.mascot-coin',
          { y: -8 },
          { type: 'spring', stiffness: 500, damping: 20, mass: 0.6 },
        )
        if (!isPlaying || !scope.current) return
        await safeAnimate('.mascot-coin', { y: 0 }, { type: 'spring', stiffness: 400, damping: 25 })
        if (!isPlaying) return
        await wait(Math.random() * 5000 + 3000)
      }
    }
    ...
    const playBlinkSequence = async () => {
      await wait(2000)
      while (isPlaying && mountedRef.current && scope.current) {
        await safeAnimate('.mascot-eye', { scaleY: 0.1 }, { duration: 0.05 })
        if (!isPlaying || !scope.current) return
        await safeAnimate('.mascot-eye', { scaleY: 1 }, { duration: 0.05 })
        if (!isPlaying) return
        await wait(Math.random() * 4000 + 2000)
      }
    }

    void playBounceSequence()
  ...
  const handleMouseEnter = () => {
    ...
    void safeAnimate('.mascot-eye', { scaleY: 0.1 }, { duration: 0.05 })
    blinkTimeout.current = setTimeout(() => {
      if (!mountedRef.current || !scope.current) return
      void safeAnimate('.mascot-eye', { scaleY: 1 }, { duration: 0.05 })
    }, 150)
  }
